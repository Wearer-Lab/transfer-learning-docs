...
<Tabs>
  <Tab title="Automated Installation">
    Use our automated installation scripts for a quick setup:

    <CodeGroup>
      <CodeBlock title="Unix (macOS/Linux)">
      ```bash
    ...
    <Note>
    After installation, you'll need to add your OpenAI API key to the `.env` file.
    </Note>
  </Tab>
  ...
  <Tab title="Manual Installation">
    <Steps>
      ...
      <Step title="Install UV Package Manager">
        ...
        <Tip>
        UV is a modern Python package manager that provides significant performance improvements over traditional pip. Transfer Learning uses UV for faster, more reliable dependency management.
        </Tip>
      </Step>
...
Transfer Learning provides a convenient command alias `tl` that can be used as a shorthand for the `transfer-learning` command. All examples in this guide can use either form:
...
<Note>
The `tl` command alias is available after installation. If you want to make it available globally, follow the instructions in the [CLI Overview](/cli/overview#global-installation).
</Note>
